...
本プロジェクトは [Maggotxy/xiaozhi-esp32-music](https://github.com/Maggotxy/xiaozhi-esp32-music) をベースに二次開発を行い、WeChatメッセージプッシュ機能を追加し、ML307 4Gネットワークでの互換性問題を修正しました。
...
- 🎵 **インテリジェント音楽制御**: `self.music.play_song` ツールで歌詞表示付き音楽再生に対応
- 💬 **WeChatメッセージプッシュ**: `self.message.send_to_wechat` ツールで音声メッセージをWeChatに送信
- 📡 Wi-Fi / ML307 Cat.1 4G
- 🗣️ オフライン音声ウェイクアップ [ESP-SR](https://github.com/espressif/esp-sr)
...
- ☁️ クラウド側MCPで大規模モデル能力を拡張（スマートホーム制御、PCデスクトップ操作、知識検索、メール送受信など）
...
小智は [WxPusher](https://wxpusher.zjiecode.com/) メッセージプッシュサービスを使用して、音声メッセージをWeChatに送信できます。

### 📱 設定手順

1. **WxPusherアカウント登録**
   - [WxPusher公式サイト](https://wxpusher.zjiecode.com/admin/main) にアクセス
   - WeChatのQRコードでスキャンして登録・ログイン

2. **アプリケーション作成とAppToken取得**
   - コンソールで新しいアプリケーションを作成
   - アプリケーションの `AppToken` をコピー

3. **ユーザーUID取得**
   - "マイページ"で `UID` を確認
   - またはWeChatで"WxPusher"公式アカウントをフォローし、対話でUIDを取得

4. **設定ファイルの修正**
   - プロジェクトルートディレクトリの `sdkconfig` ファイルを開く
   - 以下の設定項目を見つけて変更：
   ```
   ...
   CONFIG_MESSAGE_PUSHER_APP_TOKEN="あなたのAppToken"
   CONFIG_MESSAGE_PUSHER_UID="あなたのUID"
   ```
   - または `idf.py menuconfig` コマンドを使用して `Xiaozhi Assistant` メニューで設定

### 🎯 使用方法

設定完了後、以下のような音声コマンドが使用できます：
...
小智が自動的にメッセージの意図を認識し、内容を抽出してWeChatにプッシュします。

### 🔧 技術的特徴

- ✅ WiFiおよびML307 4Gネットワーク対応
- ✅ 自動URL エンコーディング、中国語コンテンツ対応
- ✅ MCPプロトコルベースの `self.message.send_to_wechat` ツール
- ✅ リアルタイムプッシュ、秒レベルの配信
