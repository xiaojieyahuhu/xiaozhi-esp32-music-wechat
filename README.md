...
- [xiaojieyahuhu/MeowEmbeddedMusicServer](https://github.com/xiaojieyahuhu/MeowEmbeddedMusicServer) - Meow 为嵌入式设备制作的音乐串流服务
  - 🎵 在线听音乐，支持多个音乐源（酷我、网易云、咪咕、百度等）
  - 📱 为嵌入式设备提供音乐串流服务
  - 📚 管理音乐库，智能缓存机制
  - 🔍 搜索和缓存音乐
  - ⬇️ 支持通过 `url=true` 参数直接下载音频文件
  - 🎼 自动获取歌词和封面
  - 💾 基于 Go 语言开发，性能优异
  - 使用示例：
    ```
    # 搜索并获取音乐信息
    http://localhost:2233/stream_pcm?song=歌曲名&artist=歌手名
    
    # 直接下载音频文件（适用于4G网络）
    http://localhost:2233/stream_pcm?song=歌曲名&artist=歌手名&url=true
    ```
